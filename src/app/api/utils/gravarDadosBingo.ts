import { Localidade, Prisma, PrismaClient, Secao } from "@prisma/client";
...
export const gravarDadosBingo = async (
  ...
  try {
    ...
    if (tx === undefined)
      return { success: false, message: "Transação não iniciada" };
    const { success, message } = await transaction(tx);
    if (!success) {
      return {
        success,
        message,
